...
public class AIController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        //}
        navNode = GameObject.Instantiate(navNodeTemplate, this.transform.position, navNodeTemplate.transform.rotation) as GameObject;

        navNode.GetComponent<EnemyMovement>().Init(this);
    ...
    private void FixedUpdate()
    {
        ...
        m_Move = (target - transform.position).normalized;
        m_Move.y = 0f;

        if ((target - transform.position).magnitude < 0.7f)
            m_Move = Vector3.zero;
    ...
    private void CheckActions()
    {
        if (!lAttack)
        {
            ...
            lAttack = (transform.position - target).magnitude < 0f;
        }
        //if (!hAttack)
